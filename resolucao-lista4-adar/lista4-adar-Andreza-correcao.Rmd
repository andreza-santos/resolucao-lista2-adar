...
> Por que include=FALSE? removeu o chunck do html. Removi para reprodutibilidade.
...
> include=FALSE nos chuncks é inaceitável. Como reproduzirei seu Rmd? 
Você provavelmente não testou seus códigos e é justamente pra isso que serve o
R Markdown.
...
> Código que interessa está comentado. 
...
dados_temp_arrumado <- mutate(
  ...
  day = gsub(pattern = "d",
             replacement = "",
             x = day, # dados_sm_ord$day #! desnecessario, dentro do mutate nao precisa
  )
)

#! porque não colocou no mutate?
...
> A idéia dos exercícios no final do capítulo é usar o que foi aprendido naquele capítulo (`mutate` ao invés de `x$var <- algocom(x$var)`). Você não ordenou os dados: arrange(year, month, day).
...
> Dentro do mutante o acesso as variáveis é direto!



```{r jdt-correcao4, echo = FALSE, comment = "JDT>"}
p4 <- (0.5 * 1/3) + (0.5 * 1/3)  + (0.5 * 1/3)
(nq4 <- 1 - p4)
```
...
# USE MUTATE, ensinado no capítulo
dados_est_sup$variavel_hora <- str_replace_all(string = dados_est_sup$variavel_hora, pattern = "[:punct:]", "_") # desnecessário, veja qual o separador default da ?separate
...
data_dados_est_sup <- unite(
  ...
  mutate(
    .,
    #! como acessar a variavel de um conj de dados que ainda nem existe?
    #! alguma vez usei a strptime em aula? as.POSIXct! veja ?strptime para saber
    #! qual é o formato de saída da strptime.
    #date = strptime(data_dados_est_sup$date, "%d-%m-%Y %H:%M:%S")
    date = as.POSIXct(date, format = "%d-%m-%Y %H:%M:%S", tz = "UTC")
  )
...
> Você não entendeu uma das principais vantagens da mutate. Dentro dela você acessa as variáveis pelo nome! não precisa `dados$nome`.
...
#! perigo, especificação de colunas por numero feito a olho
...
Misturou `<-` e `=` . Usou números inseridos a olho para definir as colunas, ao invés de nomes.
...
> No curso não foram ensinadas as funções `gather()` e `spread()`, leia o livro. Essas funções estão ultrapassadas. Estranho você usá-las aqui tendo usado `pivot_*` nas questões anteriores. Sinal de confusão. 
...
dados_temp_sm <- dados_temp_rs %>%
  filter(site == "A803") # de onde veio o A803? olhômetro? pensar como programadora
