...
prec_dia <- prec_dia %>% mutate(., date = gsub(pattern = "X", replacement = "", x = prec_dia$date))
...
dados_temp_arrumado <- dados_sm_ord %>%
  mutate(
    .,
    day = gsub(pattern = "d",
               replacement = "",
...
```{r include=FALSE}
nova_col_dados_sm <- dados_temp_arrumado %>%
  mutate(
    .,
    tmed = (dados_temp_arrumado$tmax + dados_temp_arrumado$tmin) / 2
  )
(nova_col_dados_sm %>% head(n =3))
```
...
```{r include=FALSE}
dados_sm_modif <- nova_col_dados_sm %>%
  rename(
    .,
    "ano" = year,
    "mes" = month,
    "dia" = day)
(dados_sm_modif %>% head(n =3))
```
...
```{r include=FALSE}
dados_sm_faltantes <- unir_col_dados_sm %>%
  filter(
    .,
    is.na(unir_col_dados_sm$data)
  ) 
...
Os dados são faltantes porque cada mês foi agrupado para receber 31 informações, de 31, mas o número de dias muda de acordo com o mês, assim, os dados para aqueles meses que têm menos de 31 dias são preenchidos com `NA`.
...
dados_est_sup$hora <- as.numeric(dados_est_sup$hora)
...
#   sep = " "
# )
# dados_est_sup$date <- as.POSIXct(dados_est_sup$date,
#                                       tz = ""
#                                       )
