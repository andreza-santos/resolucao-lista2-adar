...
# data_dados_est_sup$date <- as.Date(as.POSIXct(data_dados_est_sup$date, origin = "01-01-2010", tz = "UTC"))
# data_dados_est_sup$date <- as.Date(as.POSIXct(data_dados_est_sup$date, origin = "01-01-2010", tz = "UTC", format = "%tc"))
...
num_obs <- dados_rs_08_16 %>% count(site)
period <- num_obs$n / 8760
...
```{r}
dados_validos_ema <- dados_rs_08_16

n = nrow(dados_rs_08_16)
# porcentagem de NA por coluna
round(colSums(is.na(dados_rs_08_16))*100/n, 2)
colMeans(is.na(dados_rs_08_16))
n_na <- colSums(is.na(dados_rs_08_16))

dados_validos_ema <- group_by(
    dados_rs_08_16,
    site) %>%
  summarise(colMeans(is.na(dados_rs_08_16)))




# summarise(n = n()) %>%
#   mutate(tair = paste0(round(100 * n / n_na[3], 0), "%"))

# count(site) %>%
#   mutate(freq = n / na_na)
  
# mutate(tair = tair/sum(tair, na.rm = TRUE)) %>%  
# mutate(tair = round(tair*100, 2))
```
