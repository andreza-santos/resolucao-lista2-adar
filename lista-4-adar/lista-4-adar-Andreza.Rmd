...
```
```{r}
soi_arrumado <- pivot_longer(data = soi, 
             cols = -(YEAR),
             names_to = "mes",
             values_to = "soi")

unique(soi_arrumado$YEAR)
...
soi_arrumado <- soi_arrumado %>% mutate_if(is.character, as.integer)
...
```{r}
dpoluentes <- poluentes
as_tibble(dpoluentes)
...
```{r}
names(dados_sm_modif) <- c("id", "ano", "mes", "dia", "tmax", "tmin", "tmed")
```
...
```{r}
dados_sm_modif %>% unite(date, ano, mes, dia)
```
...
```{r}
dados_sm_modif <- dados_sm_modif %>% filter(date == "NA")
